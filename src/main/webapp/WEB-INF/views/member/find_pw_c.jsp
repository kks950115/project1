...
<script>
    ...
    $(function() {
        ...
        $('#cbno').keyup(function() {
            if($('#cbno').val() == "" || $('#cbno').val().length != 10) { //사업자번호 10자리 아니면
                $('.cbno_error').show();
                $('.cbno_error').css('color','red');
                $('.cbno_error').text('10자리의 사업자등록번호를 입력해주세요.');
                $('#cbno').focus();
            } else {
                $('.cbno_error').text('');
                $('.cbno_error').css('display','none');
            }
        })
        ...
        $('#signup_btn').on('click', function(e){
            ...
            if($('#cbno').val() == "" || $('#cbno').val().length != 10) { //사업자번호 10자리 아니면
                $('.cbno_error').show();
                $('.cbno_error').css('color','red');
                $('.cbno_error').text('사업자등록번호를 입력해주세요.');
                $('#cbno').focus();
                e.preventDefault();
            } 
            ...
            if($('#memberid').val() == "" && $('#cbno').val() == "" && $('#cbno').val().length != 10 && isEmail($('#email').val()) == false) {
                $('#memberid').focus();
                e.preventDefault();
            } else {
                $.ajax({
                    url: '/member/find_pw_c',
                    type:'post',
                    data: {'memberid':$('#memberid').val(), 'cbno':$('#cbno').val(), 'email':$('#email').val()},
                    success: function(result){
                        if(result == 'ok') {
                            
                            $.ajax({
                                url: '/member/sendMailForPw',
                                type: 'post',
...
<body>
    <div id="wrap">
        ...
        <div id="container">
            ...
            <div class="step">
                <ul>
                    <li><a class="change_btn" href="/member/find_id_c">아이디찾기</a></li>
            ...
            <div class="guideText">
                ...
                <h2 class="text_1">G-FAIR KOREA<br>기업회원 비밀번호찾기</h2>
            ...
            <section>
                ...
                <dl>
                    ...
                    <dd class="join_list"><input type="text" class="user_input" id="cbno" name="cbno" placeholder="사업자번호를 입력해주세요." oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"></dd>
